...
\textcolor{blue}{EQ: Updating the samples: \\ The old samples uses only login days to define a price as being part of increasing or decreasing trend: \\
...
-- Stable Price sample: Remaining days, prices were larger or smaller than $FirstLoginPrice$, but they never change the first left digit in comparison with the first left digit of $FirstLoginPrice$ \\ \\
...
\begin{figure}[hbt!]
	\caption{Leftmost Stock Price Digit and Probability of Sale \textcolor{blue}{[EQ: New samples, defining patterns based on market prices not restricted to login days]} }%
	\label{fig:left_digit_sell}%
	\centering%	
	\bigskip
	\subfigure[Price Increasing]{
...
\textbf{EQ: Right plot looks a bit different because I use a different seed in the simulation.}
